			...
			ULONG	*lpBytesReturned)
{
	...
	maped = MmMapIoSpace(address, size, (MEMORY_CACHING_TYPE)FALSE);

	error = FALSE;
	switch(param->UnitSize){
		case 1:
			READ_REGISTER_BUFFER_UCHAR((PUCHAR)maped, (PUCHAR)lpOutBuffer, param->Count);
			break;
		case 2:
			READ_REGISTER_BUFFER_USHORT((PUSHORT)maped, (PUSHORT)lpOutBuffer, param->Count);
			break;
		case 4:
			READ_REGISTER_BUFFER_ULONG((PULONG)maped, (PULONG)lpOutBuffer, param->Count);
			break;
		default:
			error = TRUE;
			break;
