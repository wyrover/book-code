...
const char APP_NAME[] = "BOGLGP - Chapter 12";
char g_title[255];
...
void SetTitle(char *demo)
{
    sprintf(g_title, "%s - %s", APP_NAME, demo);
}
...
LRESULT CALLBACK MainWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
        ...
        case 'S':
            g_glRender->SetState(CGfxOpenGL::SOFT_SHADOW);
            SetTitle("Soft Shadow");
            break;

        case 'M':
            g_glRender->SetState(CGfxOpenGL::MOTION_BLUR);
