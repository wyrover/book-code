                             ...
                             OUT PLDR_DATA_TABLE_ENTRY *NewEntry)
{
    ...
    DataTableEntry = (PLDR_DATA_TABLE_ENTRY)MmHeapAllocEx(sizeof(LDR_DATA_TABLE_ENTRY));
    ...
    Buffer = (PWSTR)MmHeapAllocEx(Length);

    if (Buffer == NULL) {
        MmHeapFreeEx(DataTableEntry);
        return FALSE;
    }
                ...
                OUT PVOID *ImageBasePA)
{
    ...
    UCHAR HeadersBuffer[SECTOR_SIZE * 4];
    PIMAGE_NT_HEADERS NtHeaders;
    ...
    Status = ArcRead(FileId, HeadersBuffer, SECTOR_SIZE * 4, &BytesRead);
...
#ifdef _WINKD_
        DbgPrint("Relocating %p -> %p %s\n", NtHeaders->OptionalHeader.ImageBase,
                 VirtualBase, FileName);
#endif
