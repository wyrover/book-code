...
// ree
    TotalSize = 2 * MM_PAGE_SIZE;
    Buffer = AllocateHeapPermanent(TotalSize);
    /*
    ...
    */
    ...
    //KernelPageTablesBuffer = &Buffer[MM_PAGE_SIZE*2];//ree
...
MempAllocatePTE(ULONG Entry, PHARDWARE_PTE *PhysicalPT, PHARDWARE_PTE *KernelPT)
{
    ...
    // Identity mapping
    *PhysicalPT = AllocateHeapPermanent(MM_PAGE_SIZE);//(PHARDWARE_PTE)&PhysicalPageTablesBuffer[PhysicalPageTables*MM_PAGE_SIZE];
    PhysicalPageTables++;
    ...
    // Kernel-mode mapping
    *KernelPT = AllocateHeapPermanent(MM_PAGE_SIZE);//(PHARDWARE_PTE)&KernelPageTablesBuffer[KernelPageTables*MM_PAGE_SIZE];
    KernelPageTables++;
