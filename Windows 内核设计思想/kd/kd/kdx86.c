...
VOID
NTAPI
KdpGetStateChange(IN PDBGKD_MANIPULATE_STATE State,
                  IN PCONTEXT Context)
{
    PKPRCB Prcb;
    ULONG i;
    ...
    if (NT_SUCCESS(State->u.Continue2.ContinueStatus)) {
        ...
        /* Loop all processors */
        for (i = 0; i < KdpKeNumberProcessors; i++) {
...
KdpSetContextState(IN PDBGKD_WAIT_STATE_CHANGE WaitStateChange,
                   IN PCONTEXT Context)
{
    PKPRCB Prcb = KdpKeGetCurrentPrcb();
              ...
              OUT PLARGE_INTEGER MsrValue)
{
    ...
    {
        /* Read from the MSR */
        MsrValue->QuadPart = KD_RDMSR(Msr);
    }
               ...
               IN PLARGE_INTEGER MsrValue)
{
    ...
    {
        /* Write to the MSR */
        KD_WRMSR(Msr, MsrValue->QuadPart);
    }
    //_SEH2_EXCEPT(EXCEPTION_EXECUTE_HANDLER)
    {
        /* Invalid MSR */
                       ...
                       OUT PULONG ActualLength)
{
    ...
    if ((BaseAddress < sizeof(KPROCESSOR_STATE)) &&
        (Processor < KdpKeNumberProcessors)) {
