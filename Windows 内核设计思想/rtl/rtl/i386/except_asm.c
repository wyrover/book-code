...
_declspec(naked) RtlpCaptureContext()
{
    _asm{
        ...
        jmp CaptureRest //tree,add
    }
}

//CaptureRest:
_declspec(naked) CaptureRest()
{
    _asm{
...
_declspec(naked) RtlpExecuteHandler()
{
    _asm{
        /* Save non-volatile */
        push ebx
...
_declspec(naked) RtlpExecuteHandler2()
{
    _asm{
        /* Set up stack frame */
        push ebp
...
_declspec(naked) RtlpExceptionProtector()
{
    _asm{
...
_declspec(naked) RtlpUnwindProtector()
{
    _asm{
        /* Assume we'll continue */
        mov eax, ExceptionContinueSearch
