...
struct _Valarray_base {
    ...
    _Valarray_base(size_t __n) : _M_first(0), _M_size(0)
    {
        _M_allocate(__n);
    }
    ~_Valarray_base()
...
struct _Gslice_Iter_tmpl {
    ...
    bool _M_done() const
    {
        return _M_indices[0] == _M_gslice._M_lengths[0];
    }
...
public:
    ...
    void operator= (const valarray<value_type>& __x) const
    {
        if (!_M_gslice._M_empty()) {
            ...
            do _M_array[__i._M_1d_idx] = __x[__i._M_step];
