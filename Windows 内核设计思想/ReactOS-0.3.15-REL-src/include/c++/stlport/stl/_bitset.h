...
struct _Base_bitset {
    ...
    void _M_do_reset()
    {
        memset(_M_w, 0, _Nw * sizeof(_WordT));
    }
...
public:
    ...
    public:
        ...
        bool operator~() const
        {
            return (*(_M_wp) & _Bitset_base::_S_maskbit(_M_bpos)) == 0;
        }
    ...
    bitset(unsigned long __val) : _STLP_PRIV _Base_bitset<_Words>(__val)
    {
        _M_do_sanitize();
    }
