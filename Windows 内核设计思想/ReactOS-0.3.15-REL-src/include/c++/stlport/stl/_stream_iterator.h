...
iterator_category(const ostream_iterator<_TpP>&)
{
...
inline output_iterator_tag _STLP_CALL
iterator_category(const ostream_iterator<_TpP, _CharT, _Traits>&)
{
    return output_iterator_tag();
}
#  endif
...
value_type(const istream_iterator< __ISI_TMPL_ARGUMENTS >&)
{
    return (_Tp*) 0;
}
...
distance_type(const istream_iterator< __ISI_TMPL_ARGUMENTS >&)
{
    return (ptrdiff_t*)0;
}
#  else
