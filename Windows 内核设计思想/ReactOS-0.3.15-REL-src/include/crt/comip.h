...
public:
    ...
    template<typename _X> _com_ptr_t(LPSTR str)
    {
        new(this) _com_ptr_t(static_cast<LPCSTR>(str), NULL);
    }
    template<typename _X> _com_ptr_t(LPWSTR str)
    {
    ...
    _com_ptr_t(const _com_ptr_t &cp) throw() : m_pInterface(cp.m_pInterface)
    {
        _AddRef();
    }
    template<typename _X> _com_ptr_t(Interface *pInterface) throw() : m_pInterface(pInterface)
    {
        _AddRef();
    }
    ...
    template<typename _X> bool operator==(Interface *p)
    {
        return (m_pInterface == p) ? true : _CompareUnknown(p) == 0;
    }
    template<typename _X> bool operator==(const _com_ptr_t &p) throw()
    {
        return operator==(p.m_pInterface);
    }
    template<typename _X> bool operator==(_com_ptr_t &p) throw()
    {
        return operator==(p.m_pInterface);
    }
