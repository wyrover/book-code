...
void get_name(char *pname);                         // Read a name from stdin
...
void duplicate_file(Record *pnewrecord, int index, FILE *pfile);
...
Record *get_person(Record *precord)
{
    ...
    get_name(precord->name);                     // read the name
...
void get_name(char *pname)
{
    fflush(stdin);
    fgets(pname, MAXLEN, stdin);        // Read the name
    size_t len = strnlen_s(pname, MAXLEN);
...
void duplicate_file(struct Record *pnewrecord, int index, FILE *pfile)
{
    // Create and open a new file
    char tempname[L_tmpnam_s];

    if (tmpnam_s(tempname, L_tmpnam_s)) {
