...
void Child_setB(struct Parent *self, int v)
{
	Parent_setB(self, v);
	printf("i am Child::setB\n");
}

BOOL Child_init(struct Child * self)
{
	SUPERCLASS_INIT(self, Parent, Child);
	((struct Object *)self)->dtor = Child_dtor;
	self->setC = Child_setC;
	((struct Parent *)self)->setB = Child_setB;
	return TRUE;
}
