                     ...
                     int       nCmdShow)
{
    ...
    hAccelTable = LoadAccelerators(hInstance, (LPCTSTR)IDC_TEST3);
...
DWORD WINAPI Thread1(LPVOID pParm)
{
    ...
    while (!exit) {
        ...
        if (g_nIndex >= ARRAYSIZE) {
            exit = TRUE;
        } else {
            g_dwTime[g_nIndex] = GetTickCount();//
            g_nIndex++;
        }
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, (LPCTSTR)IDI_TEST3);
    wcex.hCursor        = LoadCursor(NULL, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW + 1);
    wcex.lpszMenuName   = (LPCSTR)IDC_TEST3;
    ...
    case WM_COMMAND:
        ...
        case IDM_TEST:
            ...
            for (i = 0; i < ARRAYSIZE; i++) {
                //把g_dwTime[i]写入szBuffer
                wsprintf(szBuffer, "%u", g_dwTime[i]) ;
                //将g_dwTime[i]输出
                TextOut(hdc, 100 * (i % 10), 20 * (i / 10) + 20,
