...
int WINAPI _tWinMain(HINSTANCE hThis, HINSTANCE hPrev, LPTSTR szCommandLine, int iWndShow)
{
    ...
    HWND hProgress = CreateWindow(PROGRESS_CLASS, _T(""), WS_CHILD | WS_VISIBLE
                                  | WS_BORDER, 20, 165, 250, 25, hWnd, (HMENU)CONTROL_PROGRESS, wndEx.hInstance, NULL);
    ...
    HANDLE hSemaphore = CreateSemaphore(NULL, lProcessorCount, lProcessorCount, szSemaphore);
    MSG msg;
...
DWORD WINAPI StartAddress(LPVOID lpParameter)
{
    ...
    HANDLE hSemaphore = OpenSemaphore(SEMAPHORE_ALL_ACCESS, FALSE, szSemaphore);
