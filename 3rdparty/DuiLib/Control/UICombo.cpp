...
void CComboWnd::Init(CComboUI* pOwner)
{
    ...
    if (m_pLayout->GetVerticalScrollBar()) {
        m_pLayout->GetVerticalScrollBar()->SetFixedWidth(m_pOwner->GetComboWndScrollBarWidth());
    }
}
...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
        ...
        //EnsureVisible(m_pOwner->GetCurSel());
        if (m_pLayout->GetVerticalScrollBar()) {
            m_pLayout->GetVerticalScrollBar()->DoEvent(event);
        }
        return 0;
    }
...
CComboUI::CComboUI() : m_pWindow(NULL), m_iCurSel(-1), m_uButtonState(0), m_iScrollBarWidth(6)
{
...
void CComboUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if( _tcscmp(pstrName, _T("popupscrollwidth")) == 0 ) SetComboWndScrollBarWidth(_ttoi(pstrValue));
...
void CComboUI::SetComboWndScrollBarWidth(int nWidth)
{
    if (nWidth > 0) {
        m_iScrollBarWidth = nWidth;
    }
}

int CComboUI::GetComboWndScrollBarWidth()
{
