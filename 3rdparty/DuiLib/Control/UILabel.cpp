...
namespace DuiLib
{
#define LEFT_IF_CENTER_MORE 0x10000000
	...
	void CLabelUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if( _tcscmp(pstrName, _T("align")) == 0 ) {
			...
			if( _tcsstr(pstrValue, _T("lvcenter")) != NULL ) {
				m_uTextStyle &= ~(DT_TOP | DT_BOTTOM);
				m_uTextStyle = (DT_LEFT | DT_VCENTER | DT_SINGLELINE);
			}
            if( _tcsstr(pstrValue, _T("rvcenter")) != NULL ) {
                m_uTextStyle &= ~(DT_TOP | DT_BOTTOM);
                m_uTextStyle = (DT_RIGHT | DT_VCENTER | DT_SINGLELINE);
            }
            if( _tcsstr(pstrValue, _T("leftifcentermore")) != NULL ) {
                m_uTextStyle = LEFT_IF_CENTER_MORE;
            }
		}
		...
		if(!GetEnabledEffect())
		{
			...
			if( IsEnabled() ) {
                if( m_bShowHtml ){
                    ...
                    if (uTextStyle == LEFT_IF_CENTER_MORE) {
                        RECT rcCalc = {0};
                        uTextStyle = DT_CENTER | DT_VCENTER;
                        DrawText(hDC, m_sText, m_sText.GetLength(), &rcCalc, DT_CALCRECT | uTextStyle);
                        if (rcCalc.right - rcCalc.left > rc.right - rc.left) {
                            uTextStyle = DT_LEFT | DT_VCENTER | DT_END_ELLIPSIS;
                        }
                    }

                    CRenderEngine::DrawText(hDC, m_pManager, rc, m_sText, m_dwTextColor, \
                        m_iFont, DT_SINGLELINE | uTextStyle);
                }
			}
			else {
                if( m_bShowHtml ){
