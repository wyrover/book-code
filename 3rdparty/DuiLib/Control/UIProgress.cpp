...
namespace DuiLib
{
	...
	void CProgressUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
        ...
        }else if( _tcscmp(pstrName, _T("forecolor2")) == 0 ) {
            if( *pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
            LPTSTR pstr = NULL;
            DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
            SetForeColor2(clrColor);
        }else if( _tcscmp(pstrName, _T("gradientstep")) == 0 ) SetStepValue(_ttoi(pstrValue));
	...
	void CProgressUI::PaintStatusImage(HDC hDC)
	{
        ...
        } else if (m_dwForeColor){
            ::OffsetRect(&rc, m_rcItem.left, m_rcItem.top);
            if (m_dwForeColor2) {
               CRenderEngine::DrawGradient(hDC, rc, m_dwForeColor, m_dwForeColor2, !m_bHorizontal, 0);
            } else {
    ...
    void CProgressUI::SetForeColor(DWORD dwForeColor)
    {
        m_dwForeColor = dwForeColor;
        Invalidate();
    }
    void CProgressUI::SetForeColor2(DWORD dwForeColor2)
    {
        m_dwForeColor2 = dwForeColor2;
        Invalidate();
    }

    void CProgressUI::SetStepValue(int nStep)
    {
        m_nStep = nStep;
    }
