...
Gdiplus::Image * CRenderEngine::LoadGifImage(STRINGorID bitmap, LPCTSTR type/* = NULL*/, DWORD mask/* = 0*/)
{
	...
	do 
	{
		if( type == NULL )
		{
			...
			if( CPaintManagerUI::GetResourceZip().IsEmpty() )
			{
				sFile += bitmap.m_lpstr;
#if 0
				pImg = Gdiplus::Image::FromFile(sFile );
				return pImg;
#else
				HANDLE hFile = ::CreateFile(sFile.GetData(), GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, \
					FILE_ATTRIBUTE_NORMAL, NULL);
	...
	if(HGLOBAL hGMem = ::GlobalAlloc(GMEM_FIXED,dwSize))
	{
		if(LPBYTE lpByte=(LPBYTE)::GlobalLock(hGMem))
		{
			::memcpy(lpByte,pData,dwSize);
			IStream* pStream = NULL;
			if(S_OK==::CreateStreamOnHGlobal(hGMem,TRUE,&pStream) && pStream)
			{
...
void CRenderEngine::DrawImage(HDC hDC, HBITMAP hBitmap, const RECT& rc, const RECT& rcPaint,
                                    ...
                                    BYTE uFade, float fRotate, bool hole, bool xtiled, bool ytiled)
{
    ASSERT(::GetObjectType(hDC)==OBJ_DC || ::GetObjectType(hDC)==OBJ_MEMDC);
        ...
        if( !hole ) {
            ...
            if( ::IntersectRect(&rcTemp, &rcPaint, &rcDest) ) {
                if( !xtiled && !ytiled ) {
                    ...
                    rcDest.bottom -= rcDest.top;
                    if (fRotate != 0){
                        DrawRotateImage(hDC, hBitmap, rcDest.left, rcDest.top, rcDest.right, rcDest.bottom, 
                            rcBmpPart, fRotate);
                    }else{
...
bool DrawImage(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, const CDuiString& sImageName, \
		const CDuiString& sImageResType, RECT rcItem, RECT rcBmpPart, RECT rcCorner, DWORD dwMask, BYTE bFade, int nRotate,\
		bool bHole, bool bTiledX, bool bTiledY)
{
	...
	CRenderEngine::DrawImage(hDC, data->hBitmap, rcItem, rcPaint, rcBmpPart, rcCorner, data->alphaChannel, bFade, nRotate, bHole, bTiledX, bTiledY);
                                          ...
                                          LPCTSTR pStrImage, LPCTSTR pStrModify)
{
    ...
    CDuiString sValue;
    LPTSTR pstr = NULL;
	int item_wid = rc.right - rc.left;
	int item_hei = rc.bottom - rc.top;
    ...
    for( int i = 0; i < 2; ++i,image_count = 0 ) {
        ...
        while( *pStrImage != _T('\0') ) {
            ...
            if( !sValue.IsEmpty() ) {
                if( sItem == _T("file") || sItem == _T("res") ) {
					if( image_count > 0 )
						DuiLib::DrawImage(hDC, pManager, rc, rcPaint, sImageName, sImageResType,
							rcItem, rcBmpPart, rcCorner, dwMask, bFade, nRotate, bHole, bTiledX, bTiledY);
				...
				else if( sItem == _T("sub") ) {	//sub=x, sub=x,y
					int hsub = 0, vsub = 0;
					hsub = (int)_tcstoul(sValue.GetData(), &pstr, 10); ASSERT(pstr); 
					if (_tcschr(pstr, _T(','))){
						vsub = (int)_tcstol(pstr + 1, &pstr, 10);  ASSERT(pstr);    
					}
					rcBmpPart.left = item_wid * hsub;
					rcBmpPart.top = item_hei * vsub;   
					rcBmpPart.right = rcBmpPart.left + item_wid;
					rcBmpPart.bottom = rcBmpPart.top + item_hei;  
				}
                    ...
                    nRotate = _tcstol(sValue.GetData(), &pstr, 10);  ASSERT(pstr);    
                }
            }
            if( *pStrImage++ != _T(' ') ) break;
        }
	...
	return DuiLib::DrawImage(hDC, pManager, rc, rcPaint, sImageName, sImageResType,
		rcItem, rcBmpPart, rcCorner, dwMask, bFade, nRotate, bHole, bTiledX, bTiledY);
...
// with alpha channel
Gdiplus::Status GenerateGDIPlusBitmap( HBITMAP hBitmap, 
        Gdiplus::PixelFormat pixel_format, Gdiplus::Bitmap** result_out)
{
    DIBSECTION  dibsection;
    int nBytes = ::GetObject( hBitmap, sizeof( DIBSECTION ), &dibsection );
    
    Gdiplus::Status s;
    std::auto_ptr< Gdiplus::Bitmap > target;

    if(nBytes != sizeof(DIBSECTION) || dibsection.dsBm.bmBitsPixel != 32)  {
        // Bitmap with plate or non-ARGB(32bpp)
        target.reset(Gdiplus::Bitmap::FromHBITMAP(hBitmap, NULL));
    }else{
        ...
        target.reset(new Gdiplus::Bitmap(width, height, pitch, pixel_format, static_cast< BYTE* >(bits)));
    }
    if( !target.get() )
...
Bitmap* ToGray(Image* src, float opacity/* = 1.0*/)
{
	UINT cy = src->GetHeight();
	UINT cx = src->GetWidth();

	Bitmap* dst = new Bitmap(cx, cy, src->GetPixelFormat());
	Gdiplus::Graphics gc(dst);

	//float r =  0.599, g = 0.587, b = 0.144;
	float r =  0.334, g = 0.333, b = 0.333, a = opacity;
	ColorMatrix cm = {
		r,	r, r, 0, 0,
		g,	g, g, 0, 0,
		b,  b, b, 0, 0,
	...
	imgAttr.SetColorMatrix(&cm, ColorMatrixFlagsDefault, 
		ColorAdjustTypeBitmap);
	gc.DrawImage(src, Rect(0, 0, cx, cy), 
		0, 0, cx, cy,UnitPixel, &imgAttr);

	return dst;
}

bool CRenderEngine::GenerateGrayBitmap(TImageInfo*pInfo ,TImageInfo** pGrayInfo)
{
	Gdiplus::Bitmap *bitmap =NULL;
	GenerateGDIPlusBitmap(pInfo->hBitmap,PixelFormat32bppPARGB,&bitmap);
	if (bitmap)
	{
		Bitmap *grayBitmap = 	ToGray(bitmap,1.0);
		(*pGrayInfo)->alphaChannel = pInfo->alphaChannel;
		(*pGrayInfo)->dwMask = pInfo->dwMask;
		(*pGrayInfo)->sResType = pInfo->sResType;
		(*pGrayInfo)->nX = pInfo->nX;
		(*pGrayInfo)->nY = pInfo->nY;
		if (grayBitmap)
		{
			HBITMAP h;
			grayBitmap->GetHBITMAP(NULL,&h);
			(*pGrayInfo)->hBitmap = h;
			return true;
		}
	}
...
/*
 * 1. 要旋转图像的大小最好是偶数
 * 2. 图像外部留置1像素的透明边距，防止小图片的模糊失真
 */
void CRenderEngine::DrawRotateImage(HDC hDC, HBITMAP hBitmap, int x, int y, int cx, int cy, RECT rcBmpPart, float fAngle){
    using namespace Gdiplus;

    double hx = cx/2.0, hy = cy/2.0;

    Gdiplus::Graphics gh(hDC);
    do {
        if (!hBitmap) break; 

        Bitmap bmpTemp(cx, cy, PixelFormat32bppARGB);

        Gdiplus::Graphics* pGhTemp = Gdiplus::Graphics::FromImage(&bmpTemp);
        if (!pGhTemp) break;

        Gdiplus::Bitmap* pImgSrc = NULL;

        if (Gdiplus::Ok != GenerateGDIPlusBitmap(hBitmap, PixelFormat32bppPARGB, &pImgSrc)) {
            delete pGhTemp; 
            break;
        } 

        pGhTemp->SetInterpolationMode(Gdiplus::InterpolationModeHighQuality);
        pGhTemp->DrawImage(pImgSrc, Rect(0, 0, cx, cy), rcBmpPart.left, rcBmpPart.top, 
            rcBmpPart.right - rcBmpPart.left, rcBmpPart.bottom - rcBmpPart.top, UnitPixel);

        PointF ptCenter( x + hx - 0.5, y + hy - 0.5);
        Matrix mtr;
        mtr.RotateAt(fAngle, ptCenter);
        gh.SetTransform(&mtr);
        gh.DrawImage(&bmpTemp, x, y);
        gh.ResetTransform();
...
bool CRenderEngine::GetImagePixel(LPCOLORREF lpColor, CPaintManagerUI* pManager, CControlUI* pControl, LPPOINT ptPixel)
{
    bool bret = false;
    RECT rc = pControl->GetPos();
    if (! PtInRect(&rc, *ptPixel)) return false;

    POINT pt = *ptPixel;
    pt.x -= rc.left;
    pt.y -= rc.top;

    HBITMAP hControlBmp = CRenderEngine::GenerateBitmap(pManager, pControl, rc);
    if (hControlBmp) {
        int         width, height;
        LPVOID      bits;
        DIBSECTION  dibsection;
        int nBytes = ::GetObject( hControlBmp, sizeof( DIBSECTION ), &dibsection );
        ...
        if (pt.x < width && pt.y < height) {
            // BGRA -> BGRA -> BGRA
            LPCOLORREF pData = static_cast< LPCOLORREF >(bits);
            //LPCOLORREF = pData[(pt->y * cx + pt.x];
            *lpColor = pData[pt.y * width + pt.x];
            bret = true;
        }
        DeleteObject(hControlBmp);
    }

    return true;
}

HRESULT CRenderEngine::LoadImage(LPBYTE pData, DWORD dwSize, TImageInfo** pResult)
{
    LPBYTE pImage = NULL;
    int x,y,n;
    pImage = stbi_load_from_memory(pData, dwSize, &x, &y, &n, 4);
