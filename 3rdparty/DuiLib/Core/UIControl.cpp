...
/* 
 * SetPos() 意为设置当前控件的绝对坐标位置
 * SetFixedXY() 意为设置当前控件的相对坐标位置
 * SetFixedWidth()
 * SetFixedHeight()
 * 浮动控件的位置是通过SetFloatPos()生成的
 */
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if( _tcscmp(pstrName, _T("reportid")) == 0 ) {
        LPTSTR pstr = NULL;
        DWORD dwValue = _tcstoul(pstrValue, &pstr, 10); ASSERT(pstr);
        SetReportID( dwValue);
    } else if( _tcscmp(pstrName, _T("reportpos")) == 0 ) {
        LPTSTR pstr = NULL;
        DWORD dwValue = _tcstoul(pstrValue, &pstr, 10); ASSERT(pstr);
        SetReportPos( dwValue);
    }
}
...
/*
 * 如果颜色值中含有alpha通道(m_dwBackColor < 0xFF000000)
 * 颜色值需为预乘 Premultiply 后的值，参见 CRenderEngine::LoadImage()
 */
...
void CControlUI::PaintBorder(HDC hDC)
{
	if(m_dwBorderColor != 0 || m_dwFocusBorderColor != 0)
	{
		...
		else
		{
			...
			else if(m_nBorderSize > 0 && m_dwBorderColor != 0)
...
DWORD CControlUI::GetReportID()
{
    return m_dwReportID;
}

void CControlUI::SetReportID(DWORD dwValue)
{
    m_dwReportID = dwValue;
}

DWORD CControlUI::GetReportPos()
{
    return m_dwReportPos;
}

void CControlUI::SetReportPos(DWORD dwValue)
{
    m_dwReportPos = dwValue;
}
