...
CControlUI* CDialogBuilder::Create(IDialogBuilderCallback* pCallback, CPaintManagerUI* pManager, CControlUI* pParent)
{
    ...
    if (pManager) {
        ...
        for (CMarkupNode node = root.GetChild() ; node.IsValid(); node = node.GetSibling()) {
            ...
            if (_tcscmp(pstrClass, _T("Image")) == 0) {
            ...
            }// TODO: Added [1/28/2015 Y]
            else if (_tcscmp(pstrClass, _T("String")) == 0) {
                nAttributes = node.GetAttributeCount();
                LPCTSTR pControlLang = NULL;
                LPCTSTR pControlName = NULL;
                LPCTSTR pControlValue = NULL;
                ...
                if (pControlName) {
                    pManager->AddString(pControlLang, pControlName, pControlValue);
                }
            }
        } // for( CMarkupNode node =
...
CControlUI* CDialogBuilder::_Parse(CMarkupNode* pRoot, CControlUI* pParent, CPaintManagerUI* pManager)
{
    ...
    for (CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling()) {
        ...
        if (_tcscmp(pstrClass, _T("Image")) == 0 || _tcscmp(pstrClass, _T("Font")) == 0 \
            || _tcscmp(pstrClass, _T("Default")) == 0 || _tcscmp(pstrClass, _T("String")) == 0) continue;
        ...
        //树控件XML解析
            ...
            case 7:
				...
				else if(_tcscmp(pstrClass, DUI_CTR_GIF_LB) == 0 )			  pControl = new CGifAnimUI;
                break;
            ...
            case 12:
                if (_tcscmp(pstrClass, DUI_CTR_ANITABLAYOUT) == 0)            pControl = new CAnimationTabLayoutUI;
        ...
        if (node.HasAttributes()) {
            ...
            for (int i = 0; i < nAttributes; i++) {
                ...
                if (_tcscmp(node.GetAttributeName(i), _T("text")) == 0) {
                    LPCTSTR lptext = node.GetAttributeValue(i);

                    if (_tcslen(lptext) > 1 && *lptext == STRING_PREFIX) {
                        CDuiString strName = lptext + 1;
                        CDuiString strVal;
                        int nLang = pManager->GetDefaultStringLang();

                        if (pManager->GetString(nLang, strName, strVal)) {
                            pControl->SetAttribute(node.GetAttributeName(i), strVal);
                            continue;
                        }
                    }
                }
