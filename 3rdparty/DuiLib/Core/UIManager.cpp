...
HWND CPaintManagerUI::m_gMainHwnd = NULL;
DWORD CPaintManagerUI::m_gdwMainThreadId = 0;
...
m_nDefaultStringLang(UILANG_ZH_CN)
{
...
void CPaintManagerUI::Init(HWND hWnd)
{
    ...
    // 增加线程回调绑定
    if (m_gMainHwnd == NULL) {
        m_gMainHwnd = m_hWndPaint;
    }
    m_gdwMainThreadId = ::GetCurrentThreadId();
    TThread::MainThreadID = ::GetCurrentThreadId();
    TThread::wake = boost::bind(&CPaintManagerUI::WakeMainThread);    
}

DWORD CPaintManagerUI::GetMainThreadId()
{
    return m_gdwMainThreadId;
}

HWND CPaintManagerUI::GetMainWindow()
{
    return m_gMainHwnd;
}
...
int CPaintManagerUI::GetDefaultStringLang()
{
	if( m_pParentResourcePM ) return m_pParentResourcePM->GetDefaultStringLang();

	if( m_nDefaultStringLang <= UILANG__FIRST ) {
		return -1;
	}
	return m_nDefaultStringLang;
}

void CPaintManagerUI::SetStringLang(int nLang)
{
	m_nDefaultStringLang = nLang;
}

DWORD CPaintManagerUI::GetCustomStringCount() const
{
	return m_aCustomStrings.GetSize();
}
bool CPaintManagerUI::AddString(LPCTSTR pStrLang, LPCTSTR pStrName, LPCTSTR pStrVal)
{
	CDuiString sLang = pStrLang;
	int nLang = -1;
	
	if (sLang == _T("ZH_CN")) nLang = UILANG_ZH_CN;
	else if (sLang == _T("ZH_TW")) nLang = UILANG_ZH_TW;
	else if (sLang == _T("EN_US")) nLang = UILANG_EN_US;
	
	return AddString(nLang, pStrName, pStrVal);
}
bool CPaintManagerUI::AddString(int nStrLang, LPCTSTR pStrName, LPCTSTR pStrVal)
{
	TStringInfo* pStrInfo = new TStringInfo;
	if( !pStrInfo ) return false;
	::ZeroMemory(pStrInfo, sizeof(TFontInfo));

	pStrInfo->nLang = nStrLang;
	pStrInfo->sStringName = pStrName;
	pStrInfo->sStringValue = pStrVal;

	if( !m_aCustomStrings.Add(pStrInfo) ) {
		delete pStrInfo;
		return false;
	}
...
bool CPaintManagerUI::AddStringAt(int index, int nStrLang, LPCTSTR pStrName, LPCTSTR pStrVal)
{
	TStringInfo* pStrInfo = new TStringInfo;
	if( !pStrInfo ) return false;
	::ZeroMemory(pStrInfo, sizeof(TFontInfo));
	...
	if( !m_aCustomStrings.InsertAt(index, pStrInfo) ) {
		delete pStrInfo;
		return false;
	}
...
bool CPaintManagerUI::GetString(int index, CDuiString& sVal)
{
	if( index < 0 || index >= m_aCustomStrings.GetSize() ) return false;
	TStringInfo* pStrInfo = static_cast<TStringInfo*>(m_aCustomStrings[index]);
	sVal = pStrInfo->sStringValue;
	return true;
}

bool CPaintManagerUI::GetString(int nLang, LPCTSTR pStrName, CDuiString& sVal)
{
	TStringInfo* pStrInfo = NULL;
	for( int it = 0; it < m_aCustomStrings.GetSize(); it++ ) {
		pStrInfo = static_cast<TStringInfo*>(m_aCustomStrings[it]);
		if( pStrInfo->sStringName == pStrName && pStrInfo->nLang == nLang) {
			sVal = pStrInfo->sStringValue;
			return true;
		}
	}
	if( m_pParentResourcePM ) return m_pParentResourcePM->GetString(nLang, pStrName, sVal);

	return false;
}

bool CPaintManagerUI::FindString(int nLang, LPCTSTR pStrName)
{
	TStringInfo* pStrInfo = NULL;
	for( int it = 0; it < m_aCustomStrings.GetSize(); it++ ) {
		pStrInfo = static_cast<TStringInfo*>(m_aCustomStrings[it]);
	...
	if( m_pParentResourcePM ) return m_pParentResourcePM->FindString(nLang, pStrName);
	return false;
}

int CPaintManagerUI::GetStringIndex(int nLang, LPCTSTR pStrName)
{
	TStringInfo* pStrInfo = NULL;
	for( int it = 0; it < m_aCustomStrings.GetSize(); it++ ) {
		pStrInfo = static_cast<TStringInfo*>(m_aCustomStrings[it]);
...
bool CPaintManagerUI::RemoveString(int nLang, LPCTSTR pStrName)
{
	TStringInfo* pStrInfo = NULL;
	for( int it = 0; it < m_aCustomStrings.GetSize(); it++ ) {
		pStrInfo = static_cast<TStringInfo*>(m_aCustomStrings[it]);
		if( pStrInfo->sStringName == pStrName && pStrInfo->nLang == nLang){
			delete pStrInfo;
			return m_aCustomStrings.Remove(it);
		}
	}
	return false;
}

bool CPaintManagerUI::RemoveStringAt(int index)
{
	if( index < 0 || index >= m_aCustomStrings.GetSize() ) return false;
	return m_aCustomStrings.Remove(index);
}

void CPaintManagerUI::RemoveAllStrings()
{
	TStringInfo* pStrInfo = NULL;
	for( int it = 0; it < m_aCustomStrings.GetSize(); it++ ) {
		pStrInfo = static_cast<TStringInfo*>(m_aCustomStrings[it]);
	...
	m_aCustomStrings.Empty();
}

TStringInfo* CPaintManagerUI::GetStringInfo(int index)
{
	if( index < 0 || index >= m_aCustomFonts.GetSize() ) return NULL;
	TStringInfo* pStrInfo = static_cast<TStringInfo*>(m_aCustomStrings[index]);
	return pStrInfo;
}
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlFromTab(CControlUI* pThis, LPVOID pData)
{
    ...
    if( !pThis->IsKeyboardEnabled() ) return NULL;
    pInfo->pLast = pThis;
...
void CPaintManagerUI::WakeMainThread()
{
    PostMessage(m_gMainHwnd, WM_NULL, 0, 0);
}
