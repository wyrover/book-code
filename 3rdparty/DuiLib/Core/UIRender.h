...
public:
    ...
    static void HSLtoRGB(DWORD* ARGB, float H, float S, float L);
		static Gdiplus::Image * LoadGifImage(STRINGorID bitmap, LPCTSTR type = NULL, DWORD mask = 0);
    static DWORD AdjustColor(DWORD dwColor, short H, short S, short L);
    static TImageInfo* LoadImage(STRINGorID bitmap, LPCTSTR type = NULL, DWORD mask = 0);
    static HRESULT LoadImage(LPBYTE pData, DWORD dwSize, TImageInfo** pResult);
    static void FreeImage(const TImageInfo* bitmap);
    static void DrawImage(HDC hDC, HBITMAP hBitmap, const RECT& rc, const RECT& rcPaint, \
        const RECT& rcBmpPart, const RECT& rcCorners, bool alphaChannel, BYTE uFade = 255, float fRotate = 0, 
    ...
    static void DrawRotateImage(HDC hDC, HBITMAP hBitmap, int x, int y, int cx, int cy, RECT rcBmpPart, float fAngle);
    static bool GetImagePixel(LPCOLORREF lpColor, CPaintManagerUI* pManager, CControlUI* pControl, LPPOINT ptPixel);
	static bool GenerateGrayBitmap(TImageInfo*pInfo ,TImageInfo** pGrayInfo);
};
