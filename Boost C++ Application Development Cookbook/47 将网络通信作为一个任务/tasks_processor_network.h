#ifndef TASK_PROCESSOR_NETWORK_HPP
#define TASK_PROCESSOR_NETWORK_HPP

#include "../46 制作定时器并把定时器事件作为任务处理/tasks_processor_timers.h"
...
private:
        ...
        const boost::system::error_code& error)
    {
        ...
        if (!error) {
            make_task_wrapped(boost::bind(func_, new_connection))(); // Running the task
        } else {
            std::cerr << ERROR << '\n';
        }
    }
...
#endif // TASK_PROCESSOR_NETWORK_HPP