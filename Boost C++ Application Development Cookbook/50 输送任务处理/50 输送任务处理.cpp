...
public:
    ...
    {
        data_packet ret = { ++i_ };
        std::cout << "get_data ... " << i_ << std::endl;
        return ret;
    }

    static const unsigned int max_runs = 10/*000*/; // 10 000 runs
...
public:
    ...
    {
        std::cout << "send_data ... " << data.value << std::endl;
        ++i_;
...
work_queue& operator<< (work_queue&, data_packet& data)
{
...
void do_decode(const data_packet& packet)
{
    std::cout << "do_decode ... " << packet.value << std::endl;
...
void do_compress(const decoded_data& packet)
{
    std::cout << "do_compress ... " << packet.value << std::endl;
