...
// 此处Cond(Param)和Func(Param)都会计算类型，所以为不正确版本。
template <class Func, class Param, class Cond, class Fallback>
struct apply_if {
    typedef typename boost::mpl::apply<Cond, Param>::type condition_t;

    // Incorrect, metafunction is evaluted when '::type' called
...
// of boost::make_unsigned<_1> if we won't be evaluting function lazy.
// 如果用不正确的apply_if， 会执行boost::make_unsigned<float>,静态断言失败。
typedef apply_if <
boost::make_unsigned<_1>,
      float,
      boost::is_integral<_1>
