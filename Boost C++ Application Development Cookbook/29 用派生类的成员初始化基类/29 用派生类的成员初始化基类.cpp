...
public:
    fake_tasks_processor()
        : tasks_processor(logger_)  // Oops! logger_不在这里
        , logger_()
    {}

#ifdef NEWER_DEFINED1
    fake_tasks_processor()
        : logger_() // Oops! logger_不在这里
...
class fake_tasks_processor_fixed : boost::base_from_member<std::ostringstream>
...
public:
    fake_tasks_processor_fixed() : logger_t()
        , tasks_processor(logger_t::member)
    {}
};

class fake_tasks_processor2 : boost::base_from_member<std::ostringstream, 0>
