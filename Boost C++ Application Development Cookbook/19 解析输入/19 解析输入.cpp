...
public:
    ...
    {
        // We do not get over paranoid /*偏执狂患者*/, so we won't check val range
...
datetime parse_datetime(const std::string& s)
{
    ...
    datetime ret;
    //使用unsigned short 作为输出类型，需要十进制，并用2位到2位的数字
    uint_parser<unsigned short, 10, 2, 2> u2_;
    //使用unsigned short 作为输出类型，需要十进制，并用4位到4位的数字
...
//这里有一个非常有趣的方法是boost::spirit::qi::rule<const char*, void()>。它擦除类型并
//并允许在源文件编写解析器并将它们导出到头文件。
//例如：
...
//但请记住，这个类意味着编译器的优化障碍，所以当它不是必须的时候，不要使用它。


int main()
{
    //此处的u2_必须是2位的，而18中的u2_则可以1位，什么鬼？
    datetime d1 = parse_datetime("2016-01-15T10:00:00Z");
    assert(d1.year() == 2016);
    assert(d1.month() == 1);
    ...
    datetime d2 = parse_datetime("2016-01-15T10:00:00");
    assert(d2.year() == 2016);
    assert(d2.month() == 1);
    assert(d2.day() == 15);
    assert(d2.hours() == 10);
    ...
    datetime d3 = parse_datetime("2016-01-15T10:00:00+09:15");
    assert(d3.year() == 2016);
    assert(d3.month() == 1);
    assert(d3.day() == 15);
    assert(d3.hours() == 10);
    ...
    datetime d4 = parse_datetime("2016-01-15-09:15");
    assert(d4.year() == 2016);
    assert(d4.month() == 1);
    assert(d4.day() == 15);
    assert(d4.hours() == 0);
