...
bool CPaintManagerUI::SetPropertyActionParse(TProperty& nTProperty, LPCTSTR pPropertyName, LPCTSTR pType, LPCTSTR pStartValue, LPCTSTR pEndValue, int iInterval, int iTimer, int iDelay, bool bRevers, bool bLoop /*= false*/, bool bAutoStart /*= true*/)
{
    ...
    if (_tcscmp(pType, _T("int")) == 0 || _tcscmp(pType, _T("image.fade")) == 0) {
        ...
        LPTSTR pStartStr = NULL;
        nTProperty.nStartValue.rcValue.left = _tcstol(pStartValue, &pStartStr, 10);
        ASSERT(pStartStr);
        nTProperty.nStartValue.rcValue.top = _tcstol(pStartStr + 1, &pStartStr, 10);
        ASSERT(pStartStr);
        nTProperty.nStartValue.rcValue.right = _tcstol(pStartStr + 1, &pStartStr, 10);
        ASSERT(pStartStr);
        nTProperty.nStartValue.rcValue.bottom = _tcstol(pStartStr + 1, &pStartStr, 10);
        ASSERT(pStartStr);
        LPTSTR pEndStr = NULL;
        nTProperty.nEndValue.rcValue.left = _tcstol(pEndValue, &pEndStr, 10);
        ASSERT(pEndStr);
        nTProperty.nEndValue.rcValue.top = _tcstol(pEndStr + 1, &pEndStr, 10);
        ASSERT(pEndStr);
        nTProperty.nEndValue.rcValue.right = _tcstol(pEndStr + 1, &pEndStr, 10);
        ASSERT(pEndStr);
        nTProperty.nEndValue.rcValue.bottom = _tcstol(pEndStr + 1, &pEndStr, 10);
        ASSERT(pEndStr);
    } else if (_tcscmp(pType, _T("point")) == 0) {
        LPTSTR pStartStr = NULL;
        nTProperty.nStartValue.siValue.cx = _tcstol(pStartValue, &pStartStr, 10);
        ASSERT(pStartStr);
        nTProperty.nStartValue.siValue.cy  = _tcstol(pStartStr + 1, &pStartStr, 10);
        ASSERT(pStartStr);
        LPTSTR pEndStr = NULL;
        nTProperty.nStartValue.siValue.cx = _tcstol(pEndValue, &pEndStr, 10);
        ASSERT(pEndStr);
        nTProperty.nStartValue.siValue.cy  = _tcstol(pEndStr + 1, &pEndStr, 10);
        ASSERT(pEndStr);
    } else false;
