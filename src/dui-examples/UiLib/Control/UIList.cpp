...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    if (_tcscmp(pstrName, _T("header")) == 0) GetHeader()->SetVisible(_tcscmp(pstrValue, _T("hidden")) != 0);
        ...
        SIZE szCheckImg;
        LPTSTR pstr = NULL;
        szCheckImg.cx = _tcstol(pstrValue, &pstr, 10);
        ASSERT(pstr);
        szCheckImg.cy = _tcstol(pstr + 1, &pstr, 10);
        ASSERT(pstr);
        SetCheckImgSize(szCheckImg);
    } else if (_tcscmp(pstrName, _T("itemiconimgsize")) == 0) {
        SIZE szIconImg;
        LPTSTR pstr = NULL;
        szIconImg.cx = _tcstol(pstrValue, &pstr, 10);
        ASSERT(pstr);
        szIconImg.cy = _tcstol(pstr + 1, &pstr, 10);
        ASSERT(pstr);
...
void CListImageTextElementUI::SetNorImg(int iIndex, LPCTSTR pstrText)
{
    ...
    while (m_aImgNor.GetSize() < pInfo->nColumns) {
        m_aImgNor.Add(NULL);
    }
...
void CListImageTextElementUI::SetSelImg(int iIndex, LPCTSTR pstrText)
{
    ...
    while (m_aImgSel.GetSize() < pInfo->nColumns) {
        m_aImgSel.Add(NULL);
    }
...
void CListImageTextElementUI::SetCheckFlag(int iIndex, bool bStartCheck /*= TRUE*/)
{
    ...
    while (m_aStartCheck.GetSize() < pInfo->nColumns) {
        m_aStartCheck.Add((LPVOID)(&bDefault));
    }
...
void CListImageTextElementUI::SetCheck(int iIndex, bool bCheck)
{
    ...
    while (m_aCheck.GetSize() < pInfo->nColumns) {
        m_aCheck.Add((LPVOID)(&bDefault));
    }
...
void CListImageTextElementUI::SetNorCheckImg(int iIndex, LPCTSTR pstrText)
{
    ...
    while (m_aImgNorCheck.GetSize() < pInfo->nColumns) {
        m_aImgNorCheck.Add(NULL);
    }
...
void CListImageTextElementUI::SetSelCheckImg(int iIndex, LPCTSTR pstrText)
{
    ...
    while (m_aImgSelCheck.GetSize() < pInfo->nColumns) {
        m_aImgSelCheck.Add(NULL);
    }
