...
//			rc.bottom = rc.top + cyFixed;
		...
		m_pm.SetCapture();  //redrain
	}	

	m_WndShadow.Create(m_hWnd);
	m_WndShadow.SetSize(3);
	m_WndShadow.SetDarkness(120);
		...
		if( IsEnabled() ){
			...
			else
			{
				...
				if (s_context_menu_observer.m_pMainWndPaintManager != NULL)
					PostMessage(s_context_menu_observer.m_pMainWndPaintManager->GetPaintWindow(), WM_MENUCLICK, (WPARAM)(new CDuiString(GetName().GetData())), (LPARAM)GetChecked());
				ContextMenuParam param;
				param.hWnd = m_pManager->GetPaintWindow();
				param.wParam = 1;
				s_context_menu_observer.RBroadcast(param);
