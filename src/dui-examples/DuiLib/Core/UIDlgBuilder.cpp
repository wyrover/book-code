            ...
            if (pManager->GetPaintWindow()) {
                ...
                for (int i = 0; i < nAttributes; i++) {
                    ...
                    if (_tcscmp(pstrName, _T("size")) == 0) {
                        ...
                        RECT rcRichEdit = { 0 };
                        LPTSTR pstr = NULL;
                        rcRichEdit.left = _tcstol(pstrValue, &pstr, 10);
                        ASSERT(pstr);
                        rcRichEdit.top = _tcstol(pstr + 1, &pstr, 10);
                        ASSERT(pstr);
                        rcRichEdit.right = _tcstol(pstr + 1, &pstr, 10);
                        ASSERT(pstr);
                        rcRichEdit.bottom = _tcstol(pstr + 1, &pstr, 10);
                        ASSERT(pstr);
