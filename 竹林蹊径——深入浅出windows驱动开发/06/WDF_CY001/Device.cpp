...
NTSTATUS DrvClass::PnpAdd(IN PWDFDEVICE_INIT DeviceInit)
{
	...
	pnpPowerCallbacks.EvtDevicePrepareHardware	= PnpPrepareHardware_sta; // 初始化
	pnpPowerCallbacks.EvtDeviceReleaseHardware  = PnpReleaseHardware_sta; // 停止
	pnpPowerCallbacks.EvtDeviceSurpriseRemoval	= PnpSurpriseRemove_sta;  // 异常移除
	pnpPowerCallbacks.EvtDeviceD0Entry	= PwrD0Entry_sta; // 进入D0电源状态（工作状态），比如初次插入、或者唤醒
	pnpPowerCallbacks.EvtDeviceD0Exit	= PwrD0Exit_sta;  // 离开D0电源状态（工作状态），比如休眠或设备移除
...
)
{
	...
	for(i = 0; i < NumSettings; i++) {
		Pairs[i].m_hUsbInterface = WdfUsbTargetDeviceGetInterface(UsbDevice, i);// 设置接口句柄
