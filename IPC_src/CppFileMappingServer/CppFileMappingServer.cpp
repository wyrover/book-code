...
* * 3/8/2009 7:06 PM Jialiang Ge Reviewed
\***************************************************************************/
...
int _tmain()
{
	...
	// Prepare the message to be written to the file view.
	TCHAR szMessage[] = _T("Message from the first process.");

	// Write the message to the file view.
	int cbMessageBytes = (_tcslen(szMessage) + 1 /* NULL */) * sizeof(TCHAR);
	CopyMemory((PVOID)pBuf, szMessage, cbMessageBytes);
	
	_tprintf(_T("The following message is written to the shared memory:\n") \
		_T("\"%s\"\n"), szMessage);

	// Wait to stop the server.
	_tprintf(_T("Press any key to stop the server.\n"));
	_getch();
