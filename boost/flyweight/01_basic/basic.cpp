...
{
	return (user1.first_name == user2.first_name) && (user1.last_name == user2.last_name);
}
...
{
	std::string full;
	full.reserve(user.first_name.get().size() + user.last_name.get().size() + 1);
...
int main()
{
	...
	std::replace_if(users.begin(), users.end(), boost::bind(same_name, _1, anna), anna);

	std::copy(users.begin(), users.end(), std::ostream_iterator<user_entry>(std::cout, "\n"));
	return 0;
}
