...
BOOL CDib::RgbToGray()
{
    ...
    if(!IsGray())
    {
        ...
        // 计算灰度位图数据所需空间
        UINT uGrayBmpLineByte = (lWidth + 3) / 4 * 4;
        DWORD dwGrayBmpDataSize = uGrayBmpLineByte * lHeight; 

        // 计算灰度位图所需空间
        DWORD dwGrayBmpSize = sizeof(BITMAPINFOHEADER) + sizeof(RGBQUAD) * 256 + dwGrayBmpDataSize;

        // 设置灰度位图文件头
        LPBITMAPFILEHEADER lpGrayBmpFileHeader = (LPBITMAPFILEHEADER)new BYTE[sizeof(BITMAPFILEHEADER)];
        memset(lpGrayBmpFileHeader, 0, sizeof(BITMAPFILEHEADER));
        lpGrayBmpFileHeader->bfType = 0x4d42;
        lpGrayBmpFileHeader->bfSize = sizeof(BITMAPFILEHEADER) + dwGrayBmpSize;
        lpGrayBmpFileHeader->bfOffBits = sizeof(BITMAPFILEHEADER) + sizeof(BITMAPINFOHEADER)
                                          + sizeof(RGBQUAD) * 256;
        lpGrayBmpFileHeader->bfReserved1 = 0;
        lpGrayBmpFileHeader->bfReserved2 = 0;            

        // 为灰度位图分配空间，并初始化为0
        LPBYTE lpGrayBmp = (LPBYTE)new BYTE[dwGrayBmpSize];
        memset(lpGrayBmp, 0, dwGrayBmpSize);

        // 设置灰度位图信息头
        LPBITMAPINFOHEADER lpGrayBmpInfoHeader = (LPBITMAPINFOHEADER)(lpGrayBmp);
        lpGrayBmpInfoHeader->biBitCount = 8;
        lpGrayBmpInfoHeader->biClrImportant = 0;
        lpGrayBmpInfoHeader->biClrUsed = 256;
        lpGrayBmpInfoHeader->biCompression = BI_RGB;
        lpGrayBmpInfoHeader->biHeight = lHeight;
        lpGrayBmpInfoHeader->biPlanes = 1;
        lpGrayBmpInfoHeader->biSize = sizeof(BITMAPINFOHEADER);
        lpGrayBmpInfoHeader->biSizeImage = dwGrayBmpDataSize;
        lpGrayBmpInfoHeader->biWidth = lWidth;
        lpGrayBmpInfoHeader->biXPelsPerMeter = m_lpBmpInfoHeader->biXPelsPerMeter;
        lpGrayBmpInfoHeader->biYPelsPerMeter = m_lpBmpInfoHeader->biYPelsPerMeter;

        // 设置灰度位图颜色表
        LPRGBQUAD lpGrayBmpRgbQuad = (LPRGBQUAD)(lpGrayBmp + sizeof(BITMAPINFOHEADER));

        // 初始化8位灰度图的调色板信息
        LPRGBQUAD lpRgbQuad;               
        for(int k = 0; k < 256; k++)
        {
            lpRgbQuad = (LPRGBQUAD)(lpGrayBmpRgbQuad + k);
            lpRgbQuad->rgbBlue = k; 
        ...
        // 灰度位图数据处理
        BYTE r, g, b; 
        LPBYTE lpGrayBmpData = (LPBYTE)(lpGrayBmp + sizeof(BITMAPINFOHEADER) 
                                         + sizeof(RGBQUAD) * 256);
        // 进行颜色转换
        for(int i = 0; i < lHeight; i++)
        {
            for(int j = 0; j < lWidth; j++)
            {
                ...
                lpGrayBmpData[i * uGrayBmpLineByte + j] = (BYTE)(0.299 * r + 0.587 * g + 0.114 * b); 
            }
        }
        ...
        // 重新设定原位图指针指向
        m_lpBmpFileHeader = lpGrayBmpFileHeader;
        m_lpDib = lpGrayBmp;
        m_lpBmpInfo = (LPBITMAPINFO)(lpGrayBmp);
        m_lpBmpInfoHeader = lpGrayBmpInfoHeader;
        m_lpRgbQuad = lpGrayBmpRgbQuad;
        m_lpData = lpGrayBmpData;
...
//=======================================================
// 函数功能： 数据替换，不改变位图长宽
// 输入参数： LPBYTE lpData-新位图数据
//            BOOL bFlag-TRUE 彩色数据；FALSE 灰度数据。
// 返回值：   TRUE-成功；FALSE-失败
//=======================================================
BOOL CDib::ReplaceData(LPBYTE lpData,BOOL bFlag)
{
    if(bFlag)
    {
        // 替换为彩色数据
    ...
    else
    {
        ...
        if(IsGray())
        {
            // 原来已是灰度位图，只替换数据即可
            m_lpData = lpData;
            memcpy(m_lpData,lpData,GetLineByte()*GetWidth());
        }
        else
        {
            // 原来是彩色位图，需要更改位图信息
            ...
            // 替换数据
            memcpy(lpGrayBmpData,lpData,dwGrayBmpDataSize);            
