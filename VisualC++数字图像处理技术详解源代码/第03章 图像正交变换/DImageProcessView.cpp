...
BEGIN_MESSAGE_MAP(CDImageProcessView, CView)
	...
	ON_COMMAND(ID_SVD, &CDImageProcessView::OnSvd)
...
void CDImageProcessView::OnSvd()
{
	// TODO: 在此添加命令处理程序代码
	CDImageProcessDoc* pDoc = GetDocument();					//获取文档
	LPSTR lpDIB;											//指向源图像的指针
//	LPSTR    lDstLineBytes;		//新图像每行的字节数									
	LPSTR    lpSrcStartBits;                                        //指向DIB像素指针

	Mm m_matBits;
	lpDIB = (LPSTR) ::GlobalLock((HGLOBAL) pDoc->GetHObject());		//锁定DIB
	lpSrcStartBits  = pDoc->m_dib.GetBits(lpDIB);							//找到DIB图像像素起始位置
	int nWidth;												//图像宽度
	int nHeight;												//图像高度
	long nWidthBytes;
	nWidth=(int)pDoc->m_dib.GetWidth(lpDIB);									//获取图像宽度
	nHeight=(int)pDoc->m_dib.GetHeight(lpDIB);									//获取图像高度
	nWidthBytes=pDoc->m_dib.GetReqByteWidth(nWidth * 8);		// 计算图像每行的字节数
	//lDstLineBytes=pDoc->m_dib.GetReqByteWidth(nHeight * 8);  // 计算新图像每行的字节数
	DWORD SizeImage = nWidthBytes * nHeight;                   // 计算图像数据的总数
	m_matBits = zeros(1,SizeImage);//通过创建一个1XSizeImage的一维0矩阵，为矩阵变量分配内存
	m_matBits = muint8(m_matBits);//将矩阵由型转换为double型，以便将图像数据赋给矩阵
	memcpy(m_matBits.addr(),lpSrcStartBits,SizeImage);
	m_matBits = rot90(reshape(m_matBits,nWidthBytes,nHeight));
	if (nWidthBytes != nWidth)
	{
		m_matBits = m_matBits(c_p,colon(1,1,nWidth));
	}
	m_matBits = mdouble(m_matBits);// 将矩阵由unsigned char型转换为double型，以便进行运算
	::GlobalUnlock((HGLOBAL) pDoc->GetHObject());//内存解锁
	//********
	//********
	Mm mSize ;
	Mm Usvd;
	...
	Usvd = svd_U(m_matBits);
	Wsvd = svd_W(m_matBits);
	Ssvd = svd_S(m_matBits);
	int fsvdAngle;
	fsvdAngle = columns(m_matBits);
	/*
	//得到第一维长度（矩阵列长度）
	n = size(a,1.0);
	...
	//pDoc->m_dib.GetBits(lpDIB) = lpSrcStartBits；
	//AfxMessageBox(_T("对不起，不是256色位图！"));// 警告
	pDoc->SetDib();									// 更新DIB大小和调色板		
	pDoc->SetModifiedFlag(TRUE);								// 设置脏标记		
	pDoc->UpdateAllViews(NULL);									// 更新视图		
