...
DWORD OnInitDialog(HWND hDlg, WPARAM wParam, LPARAM lParam)
{
	...
	//GUI에서 __argv[i]에는 명령행 인자들이 담겨있다. __argv[0]: asd.exe __argv[1]: 인자 __argv[2]:.........
	HANDLE hPipe1=(HANDLE)atoi(__argv[1]);
	
	CloseHandle(CreateThread(NULL,0,ReadThread,(void*)hPipe1,0,NULL));
...
DWORD WINAPI ReadThread(void* p)
{
	...
	while ( 1 )
	{
		...
		if ( b == FALSE ) break;
		MessageBox(0,buf,"콘솔에서 온 파이프",MB_OK);
	}
	return 0;
}
...
/*#ifndef MY_DEBUG
	...
	//데이터를 write
	HANDLE hPipe2=(HANDLE)atoi(__argv[2]);

	DWORD len;
	WriteFile( hPipe2, senddata, strlen(senddata)+1, &len, 0);
