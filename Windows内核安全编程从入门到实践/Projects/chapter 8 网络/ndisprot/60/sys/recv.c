...
--*/
{
    ...
    while (!NPROT_IS_LIST_EMPTY(&pOpenContext->PendedReads) &&
        ...
        if (pDst == NULL) {
            //The system resource is low, you may do something
        }
        ...
        //
        SrcTotalLenght = NET_BUFFER_DATA_LENGTH(NET_BUFFER_LIST_FIRST_NB(pRcvNetBufList));
        ...
        while (BytesRemaining && (pMdl != NULL) && SrcTotalLenght) {
            ...
            BytesToCopy = MIN(BytesToCopy, SrcTotalLenght);
...
--*/
{
    ...
    while (pNetBufList != NULL) {
        ...
        do {
            ...
            if (NDIS_TEST_RECEIVE_CANNOT_PEND(ReceiveFlags) || (NoReadIRP)) {
                bAcceptedReceive = FALSE;
        ...
        if ((bAcceptedReceive == FALSE) &&
            ...
            //restore pNetBufList if this was no Read IRP case
            if (NoReadIRP) {
...
--*/
{
    ...
    while (!NPROT_IS_LIST_EMPTY(&pOpenContext->PendedReads)) {
        ...
        if (IoSetCancelRoutine(pIrp, NULL)) {
            ...
            //
            //NPROT_SLEEP(1);
            //the following code replace NPROT_SLEEP(1) to surpress prefast warning 28193
            {
                NDIS_EVENT  _SleepEvent;
                NdisInitializeEvent(&_SleepEvent);

                if (NdisWaitEvent(&_SleepEvent, 1000));
            }
            NPROT_ACQUIRE_LOCK(&pOpenContext->Lock, FALSE);
        }
    }
