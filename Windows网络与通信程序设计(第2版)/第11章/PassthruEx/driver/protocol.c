...
// BEGIN_PTUSERIO   在这个函数中，我们要初始化设备名称
...
// BEGIN_PTUSERIO			在这里移动设备名称

		// 构造下层设备名称字符串
		pAdapt->LowerDeviceName.MaximumLength = DeviceName->MaximumLength;
		pAdapt->LowerDeviceName.Length = DeviceName->Length;

		// 你看上面的pAdapt->DeviceName.Buffer指向结构的末尾，我们的pAdapt->LowerDeviceName.Buffer再向后面添加
		// 让缓冲区指向结构的最后（要在上面的pAdapt->DeviceName.Buffer之后）
...
// BEGIN_PTEX_FILTER
   ...
   // 调用过滤接收封包的函数
   //
  if(!FltFilterReceivePacket( pAdapt, Packet ))
	  return 0;
// END_PTEX_FILTER
