...
var ls_txt_pre = spawn('ls', ['-lh', 'txt/fchmodSync.txt']);  // TODO: 定义命令行‘ls -lh /txt’
/**
 * 捕获控制台输出对象stdout,输出捕获数据
 */
ls_txt_pre.stdout.on('data', function (data) {
    console.log('stdout: ' + data);
    var fd = fs.openSync('txt/fchmodSync.txt', 'a'); // TODO: 打开文件（同步方式）
    fs.fchmodSync(fd, 777); // TODO: 更改文件权限(同步方法)
    console.info('fchmodSync() Done.');
    console.info();
    fs.closeSync(fd); // TODO: 关闭文件（同步方式）
    var ls_txt_suf = spawn('ls', ['-lh', 'txt/fchmodSync.txt']);  // TODO: 定义命令行‘ls -lh /txt’
    /**
     * 捕获控制台输出对象stdout,输出捕获数据
     */
