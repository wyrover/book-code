...
// txtlink with package arg: symlink the global to the local
...
link.usage = "npm txtlink (in package dir)"
           + "\nnpm txtlink <pkg> (txtlink global into local)"
...
function link (args, cb) {
  if (process.platform === "win32") {
    ...
    if (!semver.satisfies(process.version, ">=0.7.9")) {
      var msg = "npm txtlink not supported on windows prior to node 0.7.9"
        , e = new Error(msg)
      e.code = "ENOTSUP"
  ...
  if (npm.config.get("global")) {
    return cb(new Error("txtlink should never be --global.\n"
    ...
    fs.lstat(pp, function (er, st) {
      if (er) {
        ...
        return fs.realpath(pp, function (er, real) {
          if (er) log.warn("invalid symbolic txtlink", pkg)
          else rp = real
          next()
        })
      }
...
function parseableOutput (dest, rp, cb) {
  ...
  //
  // txtlink is always effectively "long", since it doesn't help much to
