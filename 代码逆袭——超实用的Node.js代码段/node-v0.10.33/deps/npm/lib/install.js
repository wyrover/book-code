...
function installOne (target, where, context, cb) {
  // the --txtlink flag makes this a "txtlink" command if it's at the
  // the top level.
    ...
    if (er || data._id === target._id) {
      ...
      function thenLink () {
        npm.commands.link([target.name], function (er, d) {
          log.silly("localLink", "back from txtlink", [er, d])
          cb(er, [resultList(target, where, parent && parent._id)])
        })
      }

    } else {
      log.verbose("localLink", "install locally (no txtlink)", target._id)
