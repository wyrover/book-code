...
function LinkWriter (props) {
  ...
  if (!((props.type === "Link" && props.Link) ||
    ...
    throw new Error("Non-txtlink type "+ props.type)
  }
...
LinkWriter.prototype._create = function () {
  ...
  // can only change the txtlink path by clobbering
  // For hard links, let's just assume that's always the case, since
  // there's no good way to read them if we don't already know.
  if (hard) return clobber(me, lp, link)

  fs.readlink(me._path, function (er, p) {
    // only skip creation if it's exactly the same txtlink
...
function create (me, lp, link) {
  fs[link](lp, me._path, function (er) {
    // if this is a hard txtlink, and we're in the process of writing out a
    // directory, it's very possible that the thing we're linking to
    // doesn't exist yet (especially if it was intended as a symlink),
    // so swallow ENOENT errors here and just soldier in.
    // Additionally, an EPERM or EACCES can happen on win32 if it's trying
    // to make a txtlink to a directory.  Again, just skip it.
