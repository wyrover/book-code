...
DrvClass* DrvClass::GetDrvClassFromDriver(WDFDRIVER Driver)
{
    ...
    return (DrvClass*)pContext->pThis;
}
...
{
    DrvClass* pThis = GetDrvClassFromDriver(Driver);
...
VOID DrvClass::PnpSurpriseRemove_sta( IN WDFDEVICE  Device)
{
    DrvClass* pThis = DrvClass::GetDrvClassFromDevice(Device);
    ASSERT(pThis);
...
NTSTATUS DrvClass::PwrD0Entry_sta(
               IN WDFDEVICE  Device, 
...
NTSTATUS DrvClass::PwrD0Exit_sta(
                         IN WDFDEVICE  Device, 
                         IN WDF_POWER_DEVICE_STATE  PreviousState
                         )
{
...
NTSTATUS DrvClass::PnpAdd(IN PWDFDEVICE_INIT DeviceInit)
{
    ...
    pContext->pThis = this; // 通过this指针可以得到一切
    //InitDeviceContext(); // 基类不操作，子类则可以添加自己的参数，但实际上因为有驱动类这个最好的保存私有属性的地方，此操作其实已经多余。
...
// 配置默认队列，设置默认队列总是并发的。
// &config：队列配置结构体（WDF_IO_QUEUE_CONFIG）
void DrvClass::InitDefQueue(WDF_IO_QUEUE_CONFIG& config)
{
    KDBG(DPFLTR_INFO_LEVEL, "[InitDefQueue]");
    WDF_IO_QUEUE_CONFIG_INIT_DEFAULT_QUEUE(&config, WdfIoQueueDispatchParallel);
    config.EvtIoDefault  = EventDefault_sta;
}
