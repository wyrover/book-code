...
AsioDriverList *g_pAsioList = 0;

void DumpASIOList(AsioDriverList *g_pAsioList)
{
	if(g_pAsioList == NULL)return;

	char dname[64];

	printf("ASIOÇý¶¯ÁÐ±í:\n");

	for(long i = 0; i < g_pAsioList->asioGetNumDev(); i++)
	{
		if(!g_pAsioList->asioGetDriverName(i, dname, 32))
		{
			printf("\t%s\n", dname);			
		}
	}
...
IASIO* loadDriver(char *name)
{
	char dname[64];
	IASIO *pAsioDrv = NULL;

	if(g_pAsioList == NULL)
		g_pAsioList = new AsioDriverList();

	DumpASIOList(g_pAsioList);

	if(name == NULL)
	{
		// return the first one
		g_pAsioList->asioOpenDriver(0, (void**)&pAsioDrv);
		return pAsioDrv;
	}

	// return the specified asio driver.
	for(long i = 0; i < g_pAsioList->asioGetNumDev(); i++)
	{
		if(!g_pAsioList->asioGetDriverName(i, dname, 64))
		{
			if (0 == _stricmp(name, dname))
			{
				g_pAsioList->asioOpenDriver(i, (void **)&pAsioDrv);
				break;
			}
		}
	}
