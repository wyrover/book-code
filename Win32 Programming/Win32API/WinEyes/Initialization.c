...
BOOL initInstance (HINSTANCE hinst, UINT resPoolID, int nCmdShow)
{
    ...
    hbrushBlack = GetStockBrush (BLACK_BRUSH) ;
    hbrushWhite = GetStockBrush (WHITE_BRUSH) ;

    if (__argc == 4)
         hbrushIris = CreateSolidBrush (
                      RGB (atoi (__argv [1]),
                           atoi (__argv [2]),
                           atoi (__argv [3]))) ;
     else
         hbrushIris = CreateSolidBrush (RGB(128, 64, 0)) ;
...
createMainFrameWindow (HINSTANCE hinst, int nCmdShow)
{
    ...
    hwnd =
        CreateWindowEx (0,              // Extended window styles
                    ...
                    WS_THICKFRAME | WS_OVERLAPPED | WS_SYSMENU,
                                           // Window style
                    CW_USEDEFAULT,      // Horizontal position of window
                    0,                  // Vertical position of window
                    GetSystemMetrics(SM_CXSCREEN)/5,  // Window width
                    GetSystemMetrics(SM_CYSCREEN)/5,  // Window height
