...
BOOL initInstance (HINSTANCE hinst, UINT resPoolID, int nCmdShow)
{
    ...
    // Load the menus for the window.
    // Specify the window menu when creating the window.
    // This overrides the default class menu.

    hmenuClass  = LoadMenu (hinst, MAKEINTRESOURCE (IDM_CLASSMENU)) ;
    ASSERT (NULL != hmenuClass) ;

    hmenuWindow = LoadMenu (hinst, MAKEINTRESOURCE (resPoolID)) ;
    ASSERT (NULL != hmenuWindow) ;

    haccWindow = LoadAccelerators (hinst, MAKEINTRESOURCE (resPoolID)) ;
    ASSERT (NULL != haccWindow) ;
...
createMainFrameWindow (HINSTANCE hinst, int nCmdShow)
{
    ...
    hwnd =
        CreateWindowEx (0,              // Extended window styles
                    ...
                    hmenuWindow,        // Handle of menu for this window
