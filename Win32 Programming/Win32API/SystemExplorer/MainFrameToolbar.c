...
typedef struct _tagButtonMapEntry {
    UINT ID ;               // Command identifier
    UINT Index ;            // Bitmap index
} BUTTONMAPENTRY ;
...
mainFrame_CreateToolbar (HWND hwnd)
{
    ...
    for (i = 0; i < Buttons; i++) {
        ...
        tbb [i].idCommand = ButtonMap [i].ID ;     // Command identifier
        tbb [i].iBitmap   = ButtonMap [i].Index ;  // Bitmap index
    ...
    switch (pnmhdr->code) {
        ...
        case TBN_GETBUTTONINFO:
            ...
            if (pTbNotify->iItem < DIM (ButtonMapExtras)) {
                ...
                pTbNotify->tbButton.iBitmap   = ButtonMapExtras [pTbNotify->iItem].Index ;
                pTbNotify->tbButton.idCommand = ButtonMapExtras [pTbNotify->iItem].ID ;
...
void toolbar_OnNeedText (HWND hwnd, int idCtrl, LPTOOLTIPTEXT pttt)
{
    ...
    pttt->lpszText = _tcschr (Prompt, (TCHAR) '\n') ;
