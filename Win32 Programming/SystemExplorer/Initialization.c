...
#define ChildStyles (WS_CHILD | WS_VISIBLE | WS_CAPTION | WS_MINIMIZE)
...
BOOL initInstance (HINSTANCE hinst, UINT resPoolID, int nCmdShow)
{
    ...
    // Create the child windows for the system metrics and device capabilities
    hwndChild = createSystemMetrics(hinst, hwnd);
    if(NULL == hwndChild)
       return FALSE;
    UpdateWindow(hwndChild);

    hwndChild = createDevCaps(hinst, hwnd);
    if(NULL == hwndChild)
       return FALSE;
    UpdateWindow(hwndChild);
...
registerWindowClasses (HINSTANCE hinst, UINT resPoolID)
{
    ...
    // Now register the next class.  Fill in all the fields, even though
    // this is currently a bit redundant; under maintenance, anything could
    // change and this is cleaner

    if(!registerSystemMetrics(hinst))
       return FALSE;
    if(!registerDevCaps(hinst))
       return FALSE;
