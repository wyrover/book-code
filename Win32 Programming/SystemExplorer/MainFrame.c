...
static void mainFrame_OnActivate(HWND hwnd, UINT state, HWND hwndActDeact, BOOL fminized);
...
static BOOL mainFrame_OnInitMenuPopup (HWND hwnd, HMENU hmenu, UINT item, BOOL sysmenu);
...
//  MESSAGES:
//
//  WM_ACTIVATEAPP      - notification that app is activated/deactivated
...
//  WM_INITMENUPOPUP    - set check marks in view menu
...
mainFrameWndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message) {
	case WM_ACTIVATE:
	    return HANDLE_WM_ACTIVATE(hwnd, wParam, lParam, mainFrame_OnActivate);
        ...
        case WM_INITMENUPOPUP:  // Notification of menu activation
                return HANDLE_WM_INITMENUPOPUP(hwnd, wParam, lParam, mainFrame_OnInitMenuPopup);
...
//
// void mainFrame_OnActivate(HWND hwnd, UINT state, HWND hwndActDeact, BOOL fminized)
//
// hwnd: Window handle
// activate: TRUE if being activaged
//           FALSE if being deactivated
// threadID: ignored
//

static HWND appfocus = NULL;
...
mainFrame_OnCommand (HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
    switch (id) {
        ...
        case IDM_SYSMETRICS:
                ...
                // If it does exist, it is opened and brought to the top
                createSystemMetrics(GetWindowInstance(hwnd), hwnd);
                return;
        case IDM_DEVCAPS:
                // If the window does not exist, it is created
                // If it does exist, it is opened and brought to the top
                createDevCaps(GetWindowInstance(hwnd), hwnd);
                return;
...
//
//  BOOL mainFrame_OnInitMenuPopup (HWND hwnd, HMENU hmenu, UINT item, BOOL sysmenu)
//
//  hwnd        Handle of window to which this message applies
//  hmenu       Handle of popup menu being activated
//  item        Index of menu in menu bar (ignored)
//  sysmenu     TRUE if system menu (ignored)
//
//  PURPOSE:    Set the check marks in the View menu
//
//  COMMENTS:
//

static BOOL
mainFrame_OnInitMenuPopup (HWND hwnd, HMENU hmenu, UINT item, BOOL sysmenu)
{
 HWND focus = GetFocus();

 CheckMenuItem(hmenu, IDM_SYSMETRICS, MF_UNCHECKED);
 CheckMenuItem(hmenu, IDM_DEVCAPS,    MF_UNCHECKED);
 if(focus != NULL && focus != hwnd)
    { /* has focus */
     int id = GetWindowLong(focus, GWL_ID);
     switch(id)
        { /* which id */
         case IDS_SYSTEMMETRICSCLASS:
                 CheckMenuItem(hmenu, IDM_SYSMETRICS, MF_CHECKED);
                 break;
         case IDS_DEVICECAPSCLASS:
                 CheckMenuItem(hmenu, IDM_DEVCAPS, MF_CHECKED);
                 break;
        } /* which id */
    } /* has focus */
 return TRUE;
