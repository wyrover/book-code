	...
	while(1)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					hChildStatic1 = GetDlgItem(hWnd,IDC_STATIC1);
					wsprintf(temp,"%d", g_clientNum);
					SetWindowText(hChildStatic1,temp);

					hChildStatic2 = GetDlgItem(hWnd,IDC_STATIC2);
					wsprintf(temp,"%d", ++cCount);
					SetWindowText(hChildStatic2,temp);
					InvalidateRect(hChildStatic1,NULL,FALSE);
					InvalidateRect(hChildStatic2,NULL,FALSE);
