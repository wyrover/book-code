...
local function preapreGlyphQuad(swapY)
	local buffers = {}
...
local function loadTTF(fname, initialSize, fontFace, swapY)
	local obj = {}
	...
	local quad = preapreGlyphQuad(swapY)
	...
	local function drawGlyph(localMatrix, globalMatrix, code, color)
		...
		if glyph then
			...
			if type(glyphItem)=='table' then
				...
				if swapY then
					lx, ly = sx*0.5 + glyph.minx, sy*0.5 - glyph.maxy + initialSize*0.5 + 2*descent
				else
					lx, ly = sx*0.5 + glyph.minx, -sy*0.5 + glyph.maxy - initialSize*0.5 - 2*descent
				end
	...
	obj.setupProjectionMatrix = function(matrix)
		shader.use()
		shader.uniform.projectionMatrix = matrix
	end
